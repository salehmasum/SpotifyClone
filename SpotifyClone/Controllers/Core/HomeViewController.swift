...
class HomeViewController: UIViewController {
    ...
    @objc func didTapLong(_ gesture: UILongPressGestureRecognizer) {
        ...
        popUpActionSheet(with: model)
    }
    
    func popUpActionSheet(with model: AudioTrack) {
        ...
        actionSheet.addAction(UIAlertAction(title: "Add to Playlist", style: .default, handler: { [weak self]  _ in
            DispatchQueue.main.async {
                ...
                vc.selectionHandler = { playlist in
                    self?.addTrackToPlaylist(track: model, playlist: playlist)
                }
    ...
    private func configureModels(
        ...
        sections.append(.newReleases(viewModels: newAlbums.compactMap({ album in
            return NewReleasesCellViewModel(
                ...
                artistName: album.artists.first?.name ?? "Saleh Masum"
            )
        })))
        ...
        sections.append(.recommendedTracks(viewModels: tracks.compactMap({
            return RecommendedTrackCellViewModel(
                name: $0.name,
                artistName: $0.artists.first?.name ?? "Saleh",
...
extension HomeViewController: UICollectionViewDelegate, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, viewForSupplementaryElementOfKind kind: String, at indexPath: IndexPath) -> UICollectionReusableView {
        ...
        let sectionTitleName = sections[section].title
        
        header.configure(with: sectionTitleName)
        return header
    }
